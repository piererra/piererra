/* ============================================================
   nfsu2-ui.js — NFSU2 Save Editor
   Author  : Piererra
   Project : Piererra Tools
   DOM interactions, event wiring, UI rendering

   Depends on:
     nfsu2-template.js  (must load first)
     nfsu2-cars.js      (must load second)
     nfsu2-core.js      (must load third)
============================================================ */
...
(function () {
  ...
  function updateInfoBar(info) {
    ...
    if ($('sde-dropzone')) $('sde-dropzone').hidden = true;
  }

  function wireChangeFile() {
    const btn = $('sde-btn-change-file');
    if (!btn) return;
    btn.addEventListener('click', () => {
      $('sde-dropzone').hidden = false;
      $('sde-infobar').hidden = true;
      $('sde-file-input').value = '';
    });
  }
